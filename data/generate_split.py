...
####
def get_cv_split(Y, nr_splits=5,
        random_state=None, shuffle=True):
    from sklearn.model_selection import StratifiedKFold
    Y = np.array(Y)
    X = np.arange(0, Y.shape[0]) # dummy

    skf = StratifiedKFold(n_splits=nr_splits, 
...
def get_nested_cv_split(
    ...
    external_skf = StratifiedKFold(n_splits=nr_external_splits, 
                        random_state=random_state, shuffle=shuffle)
    internal_skf = StratifiedKFold(n_splits=nr_internal_splits, 
                        random_state=random_state, shuffle=shuffle)
...
####
def get_subject_info(root_dir, dataset_code_list, 
                    feat_code, shape_code, 
                    mpp_code, group_mode=None):
    """
    ...
    for dataset_code in dataset_code_list:
        slide_dir = '%s/%s/[%s]-[%s]-[%s]/' % (root_dir, feat_code, dataset_code, shape_code, mpp_code)
        if 'TCGA' in dataset_code:
            sub_slide_path, sub_slide_label = dataset.TCGA.load(slide_dir)
        elif 'CPTAC-LUAD' in dataset_code:
            sub_slide_path, sub_slide_label = dataset.CPTAC.load(slide_dir, 'CPTAC-LUAD')
        elif 'CPTAC-LSCC' in dataset_code:
            sub_slide_path, sub_slide_label = dataset.CPTAC.load(slide_dir, 'CPTAC-LSCC')
        else: 
            assert False
    ...
    if group_mode == 'luad-lscc':
        ...
        slide_label_list = [int(v) - 1 for v in slide_label_list] # ! hack to make binary
...
def generate_split(args):
    ...
    cptac_split_info = get_cv_split(
        [v[1] for v in cptac_slide_info],
        nr_splits, random_state=seed, shuffle=True)
    tcga_d_split_info = get_cv_split(
        [v[1] for v in tcga_d_slide_info],
        nr_splits, random_state=seed, shuffle=True)
    tcga_f_split_info = get_cv_split(
        [v[1] for v in tcga_f_slide_info],
        nr_splits, random_state=seed, shuffle=True)
    ...
    for split_idx in range(nr_splits):
        ...
        if train_set == 'cptac':
            ...
            subset_dict['tcga-train'] = tcga_d_slide_info[train_idx].tolist() + tcga_f_slide_info[train_idy].tolist()
            subset_dict['tcga-valid'] = tcga_d_slide_info[valid_idx].tolist() + tcga_f_slide_info[valid_idy].tolist()
    ...
    if train_set == 'cptac':
        joblib.dump(split_info, 'exp_output/split/[luad-lusc]_train=cptac_test=tcga.dat')
    if train_set == 'tcga':
        joblib.dump(split_info, 'exp_output/split/[luad-lusc]_train=tcga_test=cptac.dat')
    return
